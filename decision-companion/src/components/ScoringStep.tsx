...
export function ScoringStep() {
  ...
  const selectBase = () =>
    `w-full px-3 py-2.5 text-sm rounded-lg border border-slate-200 bg-white text-slate-900 shadow-sm
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-xs text-slate-400 font-medium">{min}{unit}</span>
          <span className={`text-sm font-bold px-3 py-1 rounded-lg shadow-sm transition-all duration-200 ${filled ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-slate-400 bg-slate-100 border border-slate-200'}`}>
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => handleScoreChange(optionId, criterionId, e.target.value)}
            className="w-full h-2 rounded-full appearance-none cursor-grab active:cursor-grabbing bg-slate-200 premium-slider"
            style={{
              background: filled
  ...
  const renderPriceInput = (optionId: string, currentScore: number | undefined) => {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-sm
        transition-all duration-200 hover:border-slate-300 hover:shadow
        ...
        <input
          ...
          max={500000}
          step={1000}
          className="w-full text-sm bg-transparent text-slate-900 focus:outline-none placeholder:text-slate-400 font-medium"
        />
      </div>
    );
  };
  ...
  // Weight badge: hierarchy based on importance
  const weightBadge = (w: number) =>
    w >= 20 ? 'bg-blue-600 text-white border-2 border-blue-700 shadow-md shadow-blue-600/20' :
      w >= 10 ? 'bg-slate-100 text-slate-700 border-2 border-slate-300 font-semibold' :
        'bg-slate-50 text-slate-400 border border-slate-200 font-medium';
  ...
  return (
    <div className="animate-fade-in">
        ...
        <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
          <div
            ...
            style={{
              ...
              boxShadow: completionPercentage > 0 ? 'inset 0 1px 2px rgba(0,0,0,0.1)' : 'none',
            }}
          >
      ...
      {/* ── TABLE ── */}
      <div className="mb-6 rounded-xl border border-slate-200 shadow-sm bg-white overflow-hidden" style={{ boxShadow: 'inset 0 1px 0 0 rgb(241 245 249)' }}>
          ...
          <table className="w-full border-collapse" style={{ minWidth: `${280 + problem.options.length * 220}px` }}>
            ...
            <thead className="sticky top-0 z-30">
              ...
              <tr className="bg-slate-50 border-b-2 border-slate-200">
                {/* Corner cell */}
                <th className="w-44 min-w-[11rem] px-4 py-6 text-left border-r border-slate-200 sticky left-0 top-0 bg-slate-50 z-40 shadow-[2px_0_8px_-2px_rgba(0,0,0,0.08)]">
                ...
                {problem.options.map((option, idx) => {
                  ...
                  return (
                    <th key={option.id} className="px-3 py-6 text-left border-r border-slate-100 last:border-r-0 sticky top-0 bg-slate-50 z-30">
                      <div className="flex items-start gap-2 mb-3">
                        <span className={`w-6 h-6 shrink-0 flex items-center justify-center rounded-lg text-xs font-bold shadow-sm transition-all duration-200
                          ${isComplete ? 'bg-emerald-500 text-white' : 'bg-blue-600 text-white'}`}>
                          {isComplete ? <CheckCircle2 className="w-3.5 h-3.5" /> : idx + 1}
                        </span>
                        <div className="min-w-0 flex-1">
                          <p className="font-bold text-slate-900 text-sm leading-tight truncate">{option.name}</p>
                          {option.description && (
                            <p className="text-xs text-slate-500 mt-0.5 leading-tight line-clamp-1">{option.description}</p>
                          )}
                        </div>
                      </div>
                      <div>
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border transition-all duration-200
                          ${isComplete ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
              ...
              <tr className="bg-slate-100 border-b border-slate-200 sticky top-[104px] z-30">
                <td className="px-4 py-3 border-r border-slate-200 sticky left-0 bg-slate-100 z-40 shadow-[2px_0_8px_-2px_rgba(0,0,0,0.08)]">
                  <div className="flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-blue-600" />
                    <span className="text-xs font-bold text-slate-700">AI Auto-Fill</span>
                  </div>
                </td>
                {problem.options.map((option) => {
                  ...
                  return (
                    <td key={option.id} className="px-3 py-3 border-r border-slate-100 last:border-r-0">
                      <div className="space-y-1.5">
                        <div className="flex gap-1.5">
                          <input
                            ...
                            className="flex-1 min-w-0 px-2.5 py-1.5 text-xs bg-white border border-slate-200
                              rounded-lg text-slate-900 placeholder:text-slate-400 shadow-sm
                              focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 
                              hover:border-slate-300 transition-all duration-200 disabled:opacity-50 disabled:bg-slate-50"
                          />
                          <button
                        ...
                        {errorMsg && (
                          <div className="text-[10px] text-rose-600 bg-rose-50 border border-rose-200 rounded-lg px-2 py-1 leading-tight">{errorMsg}</div>
                        )}
                        {showUrlInput[option.id] && (
                          <div className="flex gap-1">
                            <input
                              ...
                              className="flex-1 min-w-0 px-2 py-1 text-[10px] bg-white border border-slate-200
                                rounded-lg text-slate-900 placeholder:text-slate-400 shadow-sm
                                focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 
                                disabled:opacity-50 disabled:bg-slate-50 transition-all duration-200"
                            />
                            <button
            ...
            <tbody>
              {problem.criteria.map((criterion, rowIdx) => {
                ...
                return (
                  <tr key={criterion.id} className={`border-b border-slate-100 last:border-b-0 transition-all duration-200 hover:bg-slate-50 ${isEven ? 'bg-white' : 'bg-slate-50/30'}`}>

                    {/* Criterion label cell — sticky */}
                    <td className={`px-4 py-4 border-r border-slate-200 sticky left-0 z-20 shadow-[2px_0_8px_-2px_rgba(0,0,0,0.08)] ${isEven ? 'bg-white' : 'bg-slate-50'}`}>
                      <div className="flex items-start justify-between gap-2 mb-1">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md shrink-0 transition-all duration-200 hover:scale-105 ${weightBadge(criterion.weight)}`}>
                          {criterion.weight}%
                        </span>
                      </div>
                      {isCost && (
                        <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-rose-50 border border-rose-200 w-fit">
                          <TrendingDown className="w-3 h-3 text-rose-500" />
                          <p className="text-[10px] font-medium text-rose-600 whitespace-nowrap">lower is better</p>
                        </div>
                      )}
                    </td>
                    ...
                    {problem.options.map((option) => {
                      ...
                      return (
                        <td key={option.id}
                          className="px-3 py-3 border-r border-slate-100 last:border-r-0 align-top transition-all duration-200 group z-0">
